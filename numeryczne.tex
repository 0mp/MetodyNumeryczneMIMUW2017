\documentclass[hidelinks,a4paper,fleqn]{book}
...
\section{Funkcje nieliniowe}

W życiu numeryka często trzeba wyliczać wartości funkcji. Fajnie byłoby zastępować funkcje skomplikowane na funkcje proste. Ale jakie funkcje właściwie są proste? Matematyk powie, że trygonometryczne, ja powiem, że wielomiany. Dlaczego wielomiany?
\[
p(x) = a_0 + a_1x + a_2x^2 + \cdots + a_nx^n
\]
Wielomiany są fajne, bo żeby je wyliczyć wystarczy umieć dodawać i mnożyć, a to potrafi sprzęt (a jak ktoś ma dobre oko to nawet zauważy tutaj FMA.
...
\begin{align*}
	...
	& = \cdots
\end{align*}
Implementujemy przez interacje, zatem koszt $O(n)$. Zatem nie tylko korzystam z podstawowych operacji arytmetycznych, ale nawet robię to tanio. 

\subsection{Interpolacja wielomianowa}
No dobrze, ale co właściwie mogę robić z wielomianem? Przy użyciu wielomianu mogę przykładowo szacować inne funkcje. Zanim jednak przejdziemy do takich rzeczy zastanówmy się nad następującym problemem: ``przez zadane węzły $(x_i, y_i)$, $i: 0...n$ poprowadzić wielomian $w$ taki, że $w(x_i) = y_i$ (interpolacja Lagrange'a)"
...
\draw  plot[smooth, tension=.5] coordinates {(0.5,2.8) (2,1) (3.3,3.85)};
\end{tikzpicture}

Ten problem można uogólnić następująco: ``przez zadane węzły $(x_i, y_i)$, $i: 0...n$ poprowadzić wielomian $w$ taki, żeby $\underbrace{w^{(k)}}_{\textrm{k-ta pochodna}}(x_i) = y_i^{(k)}$" (interpolacja Hermite'a).
...
\draw  plot[smooth, tension=.5] coordinates {(0.5, 1.5) (1, 2) (2, 1.4) (3, 2.3) (3.5, 2)};
\end{tikzpicture}
...
\]
Innymi słowy taki wielomian jest jednoznacznie wyznaczony.
\end{twierdz}
Dowód:
...
\[
w(x_j) = \sum_{i=1}^{n} \alpha_i\phi_i(x_j) = y_i
\]
...
\right]
\]
Ma jednoznaczne rozwiązanie $\iff$ jego macierz jest $\neq 0$ $\iff$ rozwiązanie z zerową prawą stroną ma tylko rozwiązanie $=0$. Jeśli więc $y_i = 0$, dla $i = 0, \cdots, n$ to $w(x_i) = 0$ dla $i = 0, \cdots n$. Zatem $w(\cdot)$ ma $(n+1)$ zer więc $w \equiv 0$, bo jest wielomianem stopnia $\leq n$.

Uwagi:
\begin{itemize}
	\item Wystarczy rozwiązanie układu równań $(\star)$
	\item Postać tego układu zależy od wyboru bazy. Np jeśli $\{\phi_i\}$ - baza naturalna $\phi_0(x) = 1$, $\phi_1(x) = x$, ..., $\phi_j(x) = x^j$ to wtedy nasza macierz jest postaci:
\[
\left[
	\begin{array}{ccccc}
...
\right]
\]
(macierz Vandermonde'a). Cechy tej macierzy: gęsta, niesymetryczna. Więc wydawałoby się, że przy elimnacji Gaussa będę musiał zużyć mniej więcej $O(n^3)$ flopów.
\end{itemize}

Czy da się dokonać taniej elimnacji Gaussa dla macierzy Vandermonde'a? Spróbumy wybrać inną bazę.

\begin{obserw}
Niech $w_n(\cdot)$ - wielomian interpolacyjny Lagrange'a oparty na węzłach $x_0, \cdots x_n, a$. $w_{n-1}(\cdot)$ - wielomian interpolacyjny Lagrange'a oparty na $x_0, x_{n-1}$. Wtedy $\star = w_i(x_i) - w_{n-i}(x_i) = 0$ dla $i=0, \cdots, n-1$. Zatem ten $(\star)$ wielomian zeruje się w $n$ punktach, więc $w_n(x) - w_{n-1}(x) = b_n\underbrace{(x-x_0)\cdot \cdots \cdot (x-x{n-1})}_{\phi_n(x)}$. Czyli
\[
	w_n(x) = w_{n-1})x_ + b_n\cdot\phi_n(x) = \sum_{i=0}^{n} b_i\phi_i(x)
\]
gdzie $\phi_n(x) = 1$, $\phi_j(x) = (x-x_0) \cdot \cdots \cdot (x - x_{j-1})$, $j=1, \cdots, n$ (baza Newtona). W tej bazie układ równań znajdujący wielomian interpolacyjny Lagrange'a ma taką postać:
\[
\left[
	\begin{array}{ccc}
		\phi_0(x_0) & \cdots & \phi_0(x_n) \\
...
Jak widać coraz bardziej upraszczamy to zadanie. Pokażemy algorym wyznaczania $b_0, \cdots, b_n$ - współczynników wielomiany interpolacyjnego Lagrange'a w bazie Newtona, który nie wymaga jawnej konstrukcji macierzy. Zauważmy, że $b_i$ zależy tylko od $y_0, \cdots, y_i$ co oznacza:
\[
	b_i = f[x_0, \cdots, x_i]
\]
...
\draw [->] (fxn1) edge (fxnnl);
\draw [->] (fxn) edge (fxnnl);

\draw [->] (fx01r) edge (fx012l);
\draw [->] (fx12r) edge (fx012l);

\draw [->] (fx12r) edge (fx123l);
\draw [->] (fx23r) edge (fx123l);

\draw [->] (fx012r) edge (fx0123l);
\draw [->] (fx123r) edge (fx0123l);

\draw [->] (fx0123r) edge (fx01234l);

\draw [->] (fx0n1r) edge (fx0nl);
\draw [->] (fx0n2r) edge (fx0nl);
...
Wystarczy pokazać, że jeśli jest to prawda dla $k = n-1$, to też dla $k=n$. Niech $p_k$ - wielomian interpolujący Lagrange'a (stopnia $\leq k$) interpolujący $f$ w węzłach $x_0, \cdots, x_k$. Niech $q$ - wielomian interpolacyjny Lagrange'a (stopnia $\leq n-1$) dla węzłów $x_1, \cdots, x_n$. Wtedy:
\[
...
Co można powiedzieć gdy mam reprezentacje takich dwóch wielomianów? W wielomianie $p_n(x)$ (albo $p_1$??) współczynnik przy $x^n$ jest równy $b_n = f[x_0, \cdots, x_n]$. Natomiast współczynnik przy $x^n$ w wielomianie po drugiej stronie $(\star\star): q(x) + \cfrac{x-x_n}{x_n-x_0}(q(x) - p_n(x))$, rozpisując $q$ i $p_{n-1}$ w bazie Newtona:
\begin{align*}
...
Aby wyznaczyć wartość wielomiany interpolacyjnego Lagrange'a w bazie Newtona:
\[
	w(x) = \sum_{i=0}^{n} b_i(x-x_0)\cdot \cdots \cdot (x-x_{i-1})
\]
okazuje sie, ze można użyć do tego zmodyfikowanego algorytmu Hornera w koszcie liniowym.

\begin{twierdz}[o błędzie interpolacji]
Niech $f \in C^{n+1}[a, b]$ (zbiór funkcji na przedziale [a, b] (n+1) razy różniczkowalne), $a = x_0 < x_1 < \cdots < x_n = b$ i niech $w_n(x)$ będzie wielomianem interpolacyjnym Lagrange'a dla f na tych węzłach. Wtedy
\[
	\forall_{x \in [a, b]} \exists_{\xi \in (a, b)} f(x) - w_n(x) = \cfrac{f^{(n+1)}(\xi)}{(n+1)!}(w_n(x))
\]
gdzie $w_n(x) = (x-x_0) \cdots (x-x_n)$ (gdzie $w(x_i) = f(x_i)$ dla $i = 0, \cdots, n$). (Dowód był)
\end{twierdz}

Interpolacja Hermite'a

Znaleźć wielomian $H_n(x)$ stopnia $\leq n$ tkie, że
\[
...
\[
	m = (\sum m_i - 1)
\]
To zadanie ma dokładnie jedno rozwiązanie.
...
\]
Dla wyznaczenia wielomiany interpolacyjnego Hermite w bazie Newtona (z węzłami krotnymi) działa zmodyfikowany algorytm różnic dzielonych.
\end{document}
