...
\begin{fakt} $\ $ %todo usunąć ten hack na nową linjkę :D
	\begin{enumerate}
		\item Jeśli $A=A^T$, to oczywiście ma rozkład $A = U \cdot \Lambda \cdot U^T$, gdzie kolumny $U$ to wektory własne A, $\Delta = \begin{bmatrix}
		      \lambda_1 & 0 & 0\\ 
		      0 & \ddots & 0\\ 
		      0 & 0 & \lambda_n 
...
\begin{fakt} 
	Jeśli $\lambda$ - w. własna macierzy A, to $(\lambda - \mu)$ - wartość własna macierzy $(A - \mu I)$
	\[
		(A - \mu I)v \overbrace{=}^{v\textrm{ - wektor własny odpowiadający }\lambda} = Av - \mu v = \lambda v - \mu v = (\lambda - \mu)v
	\]
\end{fakt} 
...
Niech $f \in C^{n+1}[a, b]$\footnote{zbiór funkcji na przedziale $[a, b]$, o $(n+1)$ ciągłych pochodnych}, $a = x_0 < x_1 < \cdots < x_n = b$ i niech $w(x)$ będzie wielomianem interpolacyjnym Lagrange'a dla f na tych węzłach. Wtedy
\[
	\forall_{x \in [a, b]} \exists_{\xi \in (a, b)}\ f(x) - w(x) = \cfrac{f^{(n+1)}(\xi)}{(n+1)!}(\omega_n(x))
\]
