...
\end{array}
\]
A to jest formuła trójczłonowa na wielomiany Czybyszewa. Mimo, że na pierwszy rzut oka nie wyglądają jak wielomiany, to widać, że $T_0$ to wielomian, podobnie jak $T_1$, a pozostałe $T_k$ to sumowa i iloczyn poprzednich wielomianów, zatem to również jest wielomian.
...
\begin{fakt}
Miejsca zerowe $k$-tego wielomianu Czybyszewa są rzeczywiste, jednokrotne i leżą w przedziale $(-1, 1)$:
\[
...
\]
Wyraz $\max_{x \in [a, b]}|(x-x_0)\cdots(x-x_n)|$ jest najmniejszy dla $[a, b] = [-1, 1]$, gdy $x_0, \cdots, x_n$ - miejsca zerowe $T_{n+1}(x)!$ Dla dowolnego $[a, b]$ zachodzi analogiczny fakt dla węzłów Czybyszewa przeskalowany liniowo na $[a,b]$.
\[
...
\begin{twierdz}[Czybyszewa o alternansie]
