...
Algorytm Hornera służy do szybkiego wyznaczania wartości $w = p(x)$. Polega on na tym, aby w odpowiednim miejscu powstawiać nawiasy.
\begin{align*}
...
\begin{obserw}
Niech $w_n(\cdot)$ - wielomian interpolacyjny Lagrange'a oparty na węzłach $x_0, \cdots x_n$, natomiast $w_{n-1}(\cdot)$ - wielomian interpolacyjny Lagrange'a oparty na $x_0, \cdots x_{n-1}$. Wtedy $\star = w_n(x_i) - w_{n-1}(x_i) = 0$ dla $i=0, \cdots, n-1$. Zatem ten $(\star)$ wielomian zeruje się w $n$ punktach, więc $w_n(x) - w_{n-1}(x) = b_n\underbrace{(x-x_0)\cdot \cdots \cdot (x-x_{n-1})}_{\phi_n(x)}$. Czyli
\[
...
\[
	H_n^{(j)}(x_i) = f^{(j)}(x_i) \quad i = 0, \cdots, n \quad j = 0, \cdots, m_i
\] 
\[
	m = \sum m_i - 1
\]
...
\]
$m_i$ oznacza krotność $i$-tego węzła, tj. liczba znanych pochodnych + 1.
...
Jak widać interpolacja wielomianem wysokiego stopnia w węzłach równoodległych może dawać duży błąd, dlatego potrzebujemy innego sposobu przybliżania skomplikowanych funkcji.
...
Te warunki dają n+1 warunków, a w przypadku interpolacji splajnem ``brakuje" jeszcze dwóch zależności, aby było tyle samo warunków co stopni swobody zadania (bo stopni swobody jest $n+l$, a $l=3$).
...
\begin{twierdz}[o błędzie aproksymacji interpolacyjnym splajnem kubicznym z Hermitowskimi warunkami brzegowymi]
...
\subsection{Krzywe B-sklejane (B-spline)}
...
Co więcej ta baza ma podobne właściwości jak ta z poprzedniego przykładu!
\[
...
\begin{itemize}
	...
	\item Obliczanie wartości $\sum_{i=-\infty}^{\infty} c_i \cdot B_i^k(x)$ dla wskazanego $x$. Zwróćmy uwagę, że znów dzięki temu że funkcje bazowe mają ograniczony nośnik to dla ustalonego $x$ jest tylko skończona liczba funkcji, gdzie składnik sumy jest niezerowy. Dlatego nieskończona suma zredukuje się do skończonej sumy, bo pozostałe składniki są mnożone przez $0$.
...
\subsection{Zadanie najlepszej aproksymacji}
...
Musimy zdecydować się na jakąś normę i znowu mamy tutaj wiele możliwości, np.
...
\[
	\begin{array}{l|l}
		...
		l_{2,\rho}\ dyskretna & \sqrt{\sum_{i=0}^N |f(t_i)|^2 \rho(t_i)}
	\end{array}
\]
...
$v_1$ ``dobrze" aproksymuje funkcję $f$ w normie $L^2$ (bo mam całkować kwadrat różnicy. I mimo dużej różnicy na małym przedziale, to ze względu na małą miarę tego przedziału i tak norma daje przyzwoity wynik). Ale jednocześnie $v_1$ źle aproksymuje $f$ w normie supremum, bo na tym małym przedziale różnica jest ogromna. Mało tego! Ta funkcja $v_1$ lepiej aproksymuje w normie $L^2$ niż funkcja $v_2$, ale jednocześnie $v_2$ lepiej aproksymuje $f$ niż $v_1$ w normie supremum!

\subsubsection{Aproksymacja w przestrzeni unitarnej}

Niech $H$ - przestrzeń liniowa z iloczynem skalarnym $(x, y)$ dla $x, y \in H$

(np. $H:=\RR^n \quad (x, y) := x^Ty = \sum_i^n x_i, y_i$)
...
Niech $V \subset H$ będzie podprzestrzenią liniową w $H$. Wówczas $v^\star \in V$ jest ENA dla $f \in H \iff (f-v^\star, v) = 0 \ \forall_{v \in V}$
\end{twierdz}

(mini rysunek przestrzeni i prostopadłego wektora)
...
\begin{wniosek}
	Jeżeli $V$ jest skończonego wymiaru to ENA istnieje i jest jedyny. Gdy $\{v_i\}^n_{i=0}$ są bazą przestrzeni $V$, to $v^\star = \sum_{i=1}^n \alpha_i^\star v_i$ i współczynniki $\{\alpha_i^\star\}$ spełniają:
\end{wniosek}
...
\[
	V = span\{p_1, \cdots, p_n\} gdzie (p_i, p_j) = 0 \quad dla\ i \neq j
\]
...
ortogonalne w $L_\rho^2(-\infty, \infty)$
\[
...
ortogonalne w $L_\rho^2(-1, 1) \quad dla \rho(x) = \cfrac{1}{\sqrt{1-x^2}}$
\[
...
Określamy $T_k(x) = \cos(k\theta)$ dla $x \in [-1, 1]$, gdzie $x = \cos\theta$ (tzn. $\theta = \arccos x$). Ze wzorów trygonometrycznych mamy:
\[
...
\begin{twierdz}[Własność minimaksu]
Spośród wszystkich wielomianów stopnia $k$ postaci:
\[
	x^k + \cdots\textrm{niższe potęgi}\cdots
\]
wielomian $\frac{1}{2^{k-1}}T_k(x)$ ma najmniejszą normę maksimum na $[-1, 1]$

(tzn. $\max_{x\in[-1, 1]} |\frac{1}{2^{k-1}}T_k(x)| \leq \max_{x \in [-1, 1]}|w(x)| \quad \forall w(x) = x^k + ...$)
...
\]
Wyraz $\max_{x \in [a, b]}|(x-x_0)\cdots(x-x_n)|$ jest najmniejszy dla $[a, b] = [-1, 1]$, gdy $x_0, \cdots, x_n$ - miejsca zerowe $T_{n+1}(x)!$ Dla dowolnego $[a, b]$ zachodzi analogiczny fakt dla węzłów Czebyszewa przeskalowanych liniowo na $[a,b]$.
\[
...
\begin{twierdz}[Czebyszewa o alternansie]
 $p^\star \in \mathbb{P}_n$ jest ENA dla $f \iff$ istnieją co najmniej $(n+2)$ punkty $x_0, \cdots, x_{n+1}$ w $[a,b]$ takie, że:
 \[
 	f(x_i) - p^\star(x_i) = s \cdot (-1)^i \cdot \norm{f - p^\star}_\infty
 \]
 gdzie $s \in \{+1, -1\}$
...
\end{proof}
Po co właściwie to robimy? Norma nieskończoność to maksimum z błędu na całym przedziale, czyli jest to błąd najgorszy. Wyobraźmy sobie, że piszemy bibliotekę numeryczną, w której wyznaczamy jakąś wartość funkcji. Naturalnym sposobem jest przybliżanie jej wielomianem, bo to jest fajna prosta funkcja. Chciałbym ten wielomian wyznaczyć w taki sposób, aby spełnić z góry zadane kryterium numerycznej poprawności. Jak to zrobić, żeby było tanio i dobrze? Powiedzmy, że chcemy, żeby przynajmniej dawało błąd nie większy niż $2 \varepsilon$ od poprawnego wyniku. Czyli mam zadane kryterium. Więc muszę znaleźć taki wielomian, który to spełni. Ponieważ to ma być funkcja biblioteczna, to będzie często wykonywana i musi być tania, dlatego chciałbym mieć dla \textbf{możliwie niskiego stopnia} jak \textbf{najlepszą jakość aproksymacji}. Czy można zrobić coś lepiej niż najlepiej? Nie, dlatego to jest najlepszy wielomian.
...
\subsubsection{Interpolacja o najlepszej aproksymacji w normie supremum}
...
$\underbrace{E_n}_{\textrm{błąd najlepszej aproksymacji}} = \norm{f-p^\star}_\infty$, $p^\star$ - wielomian najlepszej aproksymacji stopnia $\leq$ n.

Niech $w$ - wielomian interpolacyjny Lagrange'a oparty na $x_0, ..., x_n$ i pytanie: jak się ma $\norm{f-w}_\infty$ do $E_n$?
...
\begin{twierdz}
	Niech $[a_0, b_0], [a_1, b_1], ...$ oznaczają przedziały lokalizujące zero w kolejnych krokach metody bisekcji. Wtedy $a_n \ra x^\star$, $b_n \ra x^\star$ oraz $|x^\star - \frac{a_n+b_n}{2}| \leq \frac{1}{2^{n+1}}|b - a|$
\end{twierdz}

\textbf{Uwagi:}
\begin{itemize}
	\item Szybkość zbieżności słabo zależy od $f$.
...
\begin{defi}[Szybkość zbieżności ciągu $x_n \ra x^\star$]
	...
	\]
	Powiemy, że szybkość zbieżności jest co najmniej liniowa, jeśli
	\[
...
\[
	0 = f(x^\star) = f(\underline{x} + \underline{h}) = f(x) + f'(x)\cdot h + f''(x)\cdot\frac{h^2}{2} + \cdots
\]
(jeśli h - małe, to pominiemy człony wyższych rzędów).

Zatem w przybliżeniu:
\[
	0 \simeq f(x) + f'(x) \cdot h \implies h = -\frac{f(x)}{f'(x)}
\]
...
\begin{twierdz}[o zbieżności metody Newtona]
...
Istnieją modyfikacje metody Newtona, które poprawiają zbieżność.
...
\subsection{Metody typu punktu stałego}
